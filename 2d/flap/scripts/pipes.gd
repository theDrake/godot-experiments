...
const MIN_PIPE_LEN := 2
const MAX_PIPE_LEN := 14
...
func _ready() -> void:
	...
	for i in range(top_pipe_len):
		set_cell(0, Vector2i(CELL_X, i), 0, Vector2i(tile_x, tile_y), 0)
	set_cell(0, Vector2i(CELL_X, top_pipe_len), 0,
			Vector2i(tile_x, tile_y - 1), 0)
